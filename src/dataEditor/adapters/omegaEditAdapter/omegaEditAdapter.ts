...
import { OmegaEditorSessions } from './sessions'
...
export class OmegaEditorAdapter implements DataEditorService {
  private heartbeat?: HeartbeatFn
  private connected = false
  private eventEmitter: EventEmitter = new EventEmitter()
  private serverPid: number = -1
  private serverInfo: IServerInfo | undefined = undefined

  private sessions = new OmegaEditorSessions()
  ...
  async connect(): Promise<void> {
    return new Promise(async (res) => {
      ...
      this.serverInfo = await testServiceConnection().catch((err) => {
        this.eventEmitter.emit('error', err)
        res()
        return undefined
      })
